...
class SearchNode:
    ...
    def incorect(self, length):
        l = self.special_piece[2] - self.special_piece[0] + 1
        if self.special_piece[0] != 0 and length > l:
            print(self)
            return True
        return False
    ...
    def __hash__(self):
        return ''.join([''.join([str(self.info[i][j]) for j in range(len(self.info[i]))])
                        for i in range(len(self.info))])
...
class Graph:
    ...
    def generate_successors(self, current_node, heuristic="trivial"):
        ...
        for obstacle in symbols:  # take every piece and try to move it in any direction
            for direction in dirs:
                ...
                if status:
                    ...
                    c = next_node.incorect(self.length)
                    if c:
                        print("parent")
                        print(current_node)
        ...
        if lin_start <= 0 and col_start >= self.exit[0] and col_end <= self.exit[1]:
            ...
            next_node.incorect(self.length)
            if not current_node.is_visited(next_node):
                list_suc.append(next_node)
        return list_suc
...
def a_star_opt(graph, file,  heuristic='trivial'):
    ...
    while len(open_l) > 0:
        ...
        heapq.heappush(closed_l, current_node)
        if graph.check_final_state(current_node):
            end_time = time()
            print_solution_in_file(file, current_node, round(end_time - start_time, 4), max_num_of_nodes,
                                   num_of_computed_nodes)
        ...
        while i < len(successors):
            ...
            if not found:
                for node in closed_l:
                    if successor == node:
                        if successor.f < node.f:
                            closed_l.remove(node)
                            heapq.heapify(closed_l)
                        else:
                            successors.remove(successor)
                            i -= 1
